...
export default function CategoryTable({
  budget, interval, index, categories, type,
}) {
  ...
  useEffect(async () => {
    ...
    const reducedCategoryList = categoryTotals.sort((a, b) => b.amount - a.amount).slice(0, categories);
  ...
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#393E46', height: `${10 + categories * 6}vh`, width: '21vw', borderRadius: '10px',
    }}
    >
      <p style={{ color: '#D90166', margin: 0 }}>{type === 'expense' ? 'Top Expense Categories' : 'Top Income Histories'}</p>
      <div style={{
        height: '100vh', width: '20vw', display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}
      >
