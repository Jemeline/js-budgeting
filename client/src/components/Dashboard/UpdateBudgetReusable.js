...
function UpdateBudgetReusable({
  BudgetType, BudgetDescription, BudgetAmount, BudgetDate, BudgetCategory, BudgetSubcategory, BudgetClass, BudgetId,
}) {
  const [budgetType, setBudgetType] = useState((BudgetType) || 'expense');
  const [budgetDescription, setBudgetDescription] = useState((BudgetDescription) || '');
  const [budgetAmount, setBudgetAmount] = useState((BudgetAmount) || '');
...
export const colourStyles = {
  control: (styles) => ({
    ...styles, backgroundColor: 'transparent', borderColor: 'transparent', width: '20vw',
  }),
  option: (styles, {
