...
function VerifyEmail({ height }) {
  ...
  async function handleToken() {
    try {
      const body = { token, _userId: sessionStorage.getItem('id') };
  ...
  return (
    <div>
      <div style={{
        height: (height === 'modal') ? '' : 'calc(100vh - 60px)', width: '25vw', display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}
      >
        <Col>
          ...
          <Collapse in={alertInvalidToken}>
            <Alert severity="error" onClose={() => setAlertInvalidToken(false)}>
              Your token is incorrect or expired. Please
              {' '}
              <Link to="/verify" onClick={() => { setStep(0); }}>request a new token</Link>
              {' '}
              or try again.
            </Alert>
          </Collapse>
          <Collapse in={alertSuccess}>
            <Alert severity="success" onClose={() => setAlertSuccess(false)}>
              Success!
              {' '}
              <Link to="/verify" onClick={() => { history.push('/'); }}>Go to your dashboard</Link>
            </Alert>
          </Collapse>
          <div
            hidden={step !== 0}
            style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', marginTop: '10px',
            }}
          >
